...
def responsiveTest(tsDic, consecBins):
    #arrayFreq_E = cutArray(tRange, ranT[0], ranT[1], arrayFreqT_all)
    ...
    bn = 0
    #p = []
    #while bn + (consecBins-1) < len(tsDic.keys()):
    while bn + (consecBins-1) < len(tsDic):
        p = []
        p = [tsDic.loc[bn + n,'p'] for n in range(consecBins)]
        respT += [0 if (False in (np.array(p) < 0.05)) else 1]
        
        bn += 1
    responsiveness = 1 if (1 in respT) else 0

    return responsiveness

# In[]

def consFTest(arrayList):
    #at each bin
    tempDic = {}
    bn = 0
    while bn < np.shape(arrayList[0])[-1]:
        
        arrayListT = [i[:,bn] for i in arrayList]
        
        fout = stats.f_oneway(*arrayListT)
        tempDic[bn] = fout
        bn += 1
    return tempDic
...
def consTukeyHSD(arrayList):
    ...
    while bn < np.shape(arrayList[0])[-1]:
        ...
        arrayListT = [i[:,bn] for i in arrayList]
        hsdout = stats.tukey_hsd(*arrayListT)
        
        dfT = {}
        for ((i, j), l) in np.ndenumerate(hsdout.statistic):
            # filter out self comparisons
            if i != j:
                md = hsdout.statistic[i,j]
                p = hsdout.pvalue[i,j]
                dfT[str(i)+str(j)] = {'md':md, 'p':p}
                
        dfT = pd.DataFrame(dfT).T
        dfT['contrast'] = dfT.index
        
        tempDic[bn] = dfT#.reset_index(drop=True)
        
        bn += 1
    return tempDic

# In[]
def consANOVA(trial_df, epoch, arrayFreq_E, ivs):
    anovaDic = {}
    pairsDic = {}
    
    fml = 'dv ~ ' + '*'.join(['C('+ i +')' for i in ivs])
    
    bn = 0
    while bn < arrayFreq_E.shape[-1]:
        arrayT = arrayFreq_E[:,bn]
        tempDf = pd.DataFrame(columns = ['idx','dv'])
        
        tempDf['trial_index'] = trial_df['trial_index']
        tempDf['dv'] = arrayT
        
        for i in ivs:
            tempDf[i] = trial_df['stimulus_' + epoch[-1] + '_on_' + i]
        
        lmT = ols(fml, data = tempDf).fit()
        #lmT.summary()
        anovaT = sm.stats.anova_lm(lmT, typ=3)
        bonfT = pg.pairwise_tests(data = tempDf, dv = 'dv', between = ivs, padjust= 'bonf')
        anovaDic[bn] = anovaT
        pairsDic[bn] = bonfT
        
        bn += 1
        
    return anovaDic, pairsDic
...
# In[]
def selectiveTest(ftestDic, consecBins):
    #arrayFreq_E = cutArray(tRange, ranT[0], ranT[1], arrayFreqT_all)
    ...
    bn = 0
    #p = []
    while bn + (consecBins-1) < len(ftestDic.keys()):
        p = []
        p = [ftestDic[bn + n][-1] for n in range(consecBins)]
        respT += [0 if (False in (np.array(p) < 0.05)) else 1]
        
        bn += 1
    selectiveness = 1 if (1 in respT) else 0

    return selectiveness

# In[]
def consSelective(anovaDicT, consecBins, cons):
    #arrayFreq_E = cutArray(tRange, ranT[0], ranT[1], arrayFreqT_all)
    ...
    bn = 0
    #p = []
    while bn + (consecBins-1) < len(anovaDicT):
        p = []
        p = [anovaDicT[bn + n].loc[cons,'PR(>F)'] for n in range(consecBins)]
        respT += [0 if (False in (np.array(p) < 0.05)) else 1]
...
# In[]
def consANOVA_mean(trial_df, epoch, arrayFreq_E, ivs):
    #anovaDic = {}
    #pairsDic = {}
    ...
    #bn = 0
    #while bn < arrayFreq_E.shape[-1]:
    arrayT = arrayFreq_E.mean(axis = 1)
    tempDf = pd.DataFrame(columns = ['idx','dv'])
...
# In[]
def consAOV_mean(trial_df, epoch, arrayFreq_E, ivs):
    #anovaDic = {}
    #pairsDic = {}
    ...
    return anovaT
# In[]
def consPairT_mean(trial_df, epoch, arrayFreq_E, ivs):
    #anovaDic = {}
    #pairsDic = {}
...
# In[]
def consAOV_bin(trial_df, epoch, arrayFreq_E, ivs):
    anovaDic = {}
    #pairsDic = {}
...
# In[]
def consPairT_bin(trial_df, epoch, arrayFreq_E, ivs):
    #anovaDic = {}
    pairsDic = {}
...
def bslTTest_Slide_mean(arrayFreq_E, bslA, consecBins):
    
    tempDic = {}
    
    bn = 0
    while bn + consecBins < np.shape(arrayFreq_E)[-1]:
        
    #for c in range(np.shape(arrayFreq_E)[-1]):
        tout = stats.ttest_ind(arrayFreq_E[:,bn:bn+consecBins].mean(axis = 1), bslA, alternative = 'two-sided')
        tempDic[bn] = {'t':float(tout[0]), 'p':float(tout[-1])}
        bn += 1
...
# In[]
def consAOV_mean2(trial_df, arrayFreq_E, ivs):
    #anovaDic = {}
    #pairsDic = {}
    ...
    return anovaT
# In[]
def consPairT_mean2(trial_df, arrayFreq_E, ivs):
    #anovaDic = {}
    #pairsDic = {}
    ...
    return bonfT
# In[]
def consSelective_mean(anovaT, cons):
    #arrayFreq_E = cutArray(tRange, ranT[0], ranT[1], arrayFreqT_all)
    ...
    selD = {}
    for con in cons:        
        selD[con] = 1 if (np.array(anovaT.loc[con,'PR(>F)']) < 0.05) else 0
...
def consPairT_mean3(df, dv, ivs):
    ...
    #lmT.summary()
    #anovaT = sm.stats.anova_lm(lmT, typ=3)
    bonfT = pg.pairwise_tests(data = df, dv = dv, between = ivs, padjust= 'bonf')
...
def pairedVS(df, dv, ivs):

    ivL = []
    for iv in ivs:
        ivL += [sorted(df[iv].unique())]
        
    contrastList = []
    contrastList += [item for item in itertools.product(*ivL)]

    #contrastList = [item[0] + str(item[1]) for item in contrastList]
    contrastList = list(itertools.combinations(contrastList, 2))
    
    results = pd.DataFrame(columns=['prefComb1','type1', 'prefComb2', 'type2', 't', 'pAdj', 'd'])
    
    #tT = []
    pT = []
    
    for p in range(len(contrastList)):
        pairs = contrastList[p]
        temp1 = df[(df['prefComb'] == pairs[0][0]) & (df['type'] == pairs[0][1])]
        temp2 = df[(df['prefComb'] == pairs[1][0]) & (df['type'] == pairs[1][1])]
        
        results.loc[p,'prefComb1'] = pairs[0][0]
        results.loc[p,'type1'] = pairs[0][1]
        results.loc[p,'prefComb2'] = pairs[1][0]
        results.loc[p,'type2'] = pairs[1][1]
        
        tt = stats.ttest_ind(temp1.rd2, temp2.rd2)[0]
        pt = stats.ttest_ind(temp1.rd2, temp2.rd2)[1]
        
        results.loc[p,'t'] = tt
        
        results.loc[p,'d'] = cohenD(temp1.rd2, temp2.rd2)
        
        
        #tT += [tt]
        pT += [pt]
    
    pCrr = multipletests(pT, alpha=0.05, method='bonferroni')[1]
    results.loc[:,'pAdj'] = pCrr
    
    return results
    
# In[]

def pairedVS_cus(df, dv, ivs):
